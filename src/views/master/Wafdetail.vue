    ...
    <Card title="规则详情" shadow style>
      <div>
        ...
        >
          ...
          <FormItem label="规则类型" prop="policy_type">
            <!-- <Input
              :disabled="disabled"
              type="text"
            ...
            <Select :disabled="disabled" v-model="form.policy_type">
              ...
              <Option value="拒绝服务(DOS类)">拒绝服务(DOS类)</Option>
              <Option value="扫描器(Scanner)">扫描器(Scanner)</Option>
              <Option value="文件包含(LFI/RFI)">文件包含(LFI/RFI)</Option>
              <Option value="远程代码执行(RCE)">远程代码执行(RCE)</Option>
              <Option value="脚本后门(Webshell)">脚本后门(Webshell)</Option>
              <Option value="文件上传(FileUpload)">文件上传(FileUpload)</Option>
              <Option value="服务端请求伪造(SSRF)">服务端请求伪造(SSRF)</Option>
              <Option value="客户端请求伪造(CSRF)">客户端请求伪造(CSRF)</Option>
              <Option value="SQL注入(SqlInjection)">SQL注入(SqlInjection)</Option>
              <Option value="代码泄露(CodeLeakages)">代码泄露(CodeLeakages)</Option>
              <Option value="LDAP注入(LDAP Injection)">LDAP注入(LDAP Injection)</Option>
              <Option value="路径遍历(Path Traversal)">路径遍历(Path Traversal)</Option>
              <Option value="应用类(DRUPAL, WORDPRESS...)">应用类(DRUPAL, WORDPRESS...)</Option>
              <Option value="Xpath解析器注入(Xpath Injection)">Xpath解析器注入(Xpath Injection)</Option>
              ...
              <Option
                value="认证和访问控制(Authentication And Access Control)"
              >认证和访问控制(Authentication And Access Control)</Option>
              <Option
                value="Xml外部实体注入(XXE)(XML External Entity Injection)"
              >Xml外部实体注入(XXE)(XML External Entity Injection)</Option>
              <Option value="其他类型(Other types)">其他类型(Other types)</Option>
            </Select>
          </FormItem>
          <FormItem label="规则描述" prop="policy_describe">
            <Input
              :disabled="disabled"
              type="text"
          ...
          <FormItem
            label="正则表达式"
            prop="regex_policy"
            :rules="{required: true, message: '正则表达式不能为空', trigger: 'blur'}"
          >
            <Input
              :disabled="disabled"
          ...
          <!-- <div style="backgroung:#999" v-for="(item,index) in form.tactics" :key="'b'+index">
            ...
            <FormItem
              ...
              :rules="{required: true, message: '请输入正则规则', trigger: 'blur'}"
            >
              <Input
                :disabled="disabled"
